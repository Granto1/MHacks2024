...
public class LEDManager : MonoBehaviour  //ENABLE CONTROLLER ROTATION
{
    ...
    void Start()
    {
        ...
        s.addToLEDs(gameObject);

        current = transform.GetChild(1).GetComponent<MeshRenderer>().material;
    }
    ...
    public void changeLight(bool on)
    {
        if (on)
        {
            transform.GetChild(1).GetComponent<MeshRenderer>().material = bright;
        }
        else transform.GetChild(1).GetComponent<MeshRenderer>().material = current;
    }
    ...
    private void onDrop(SelectExitEventArgs args)
    {
        if (currentPinNeg != null && currentPinPos != null)
        {
            ...
            transform.position = new Vector3(midpoint.x, midpoint.y + GetTotalHeight()/3, midpoint.z);
